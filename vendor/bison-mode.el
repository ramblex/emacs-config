...
;; *************** user-definable vars ***************
(defvar bison-rule-separator-column 4
  "column for rule and production separators \"|\" and \";\"")
(defvar bison-rule-enumeration-column 6
...
(defvar bison-decl-token-column 16
...
(modify-syntax-entry ?\% "> 12b" bison-syntax-table)
...
(define-derived-mode bison-mode c++-mode "Bison"
  ...
  (setq indent-region-function 'bison-indent-region)
  ...
  (set-syntax-table bison-syntax-table)
...
(defun bison--production-end-p (bol eol section)
  "Return t if the current line is the end of a production rule"
  (save-excursion
    (goto-char bol)
    (if (and (not (bison--within-braced-c-expression-p section))
...
(defun tm ()
  (interactive)
  (print (bison--within-braced-c-expression-p (bison--section-p))))
...
(defun bison-indent-region (start end &optional to-column)
  "Indent a region of bison code"
	...
	(cond
	 ...
	 ((bison--production-alternative-p bol eol section)
	  ...
	  (funcall reset-pt))
         ...
         ((bison--production-end-p bol eol section)
          (back-to-indentation)
          (save-excursion
            (just-no-space)
            (indent-to-column bison-rule-separator-column))
